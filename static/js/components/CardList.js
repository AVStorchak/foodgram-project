class CardList extends MainCards{
    ...
    tooltipAdd  () {
        const item = this.target.closest('.card').querySelector('.card__favorite-tooltip');
        item.textContent = "Добавить в избранное"
    }
    tooltipDel () {
        const item = this.target.closest('.card__favorite').querySelector('.card__favorite-tooltip');
